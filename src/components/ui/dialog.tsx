...
// Every dialog in the app is a bottom sheet, at every viewport width — there is no
// centered-modal variant and no viewport branch. On desktop the sheet is capped to
// the same column the pages use (CONTENT_COLUMN) so it rises inside the app's column
// instead of spanning the whole monitor.
...
// Cancel a vaul "interact outside" dismissal when the event originates from an
...
function Dialog({
  ...
  // Hide the bottom nav while a dialog is actually open, so it doesn't show through
  // beneath the sheet. Tie to `open` (not mount) because the sheet stays mounted
  // while closed.
  React.useEffect(() => {
    if (!open) return;
...
function DialogContent({
  ...
  const wash = accentColor ? (
    <div
      aria-hidden
      className="absolute inset-x-0 top-0 pointer-events-none h-72"
  ...
  return (
    <Drawer.Portal>
      <Drawer.Overlay className={cn("fixed inset-0 z-50 bg-black/25 backdrop-blur-[22px]", overlayClassName)} />
      <Drawer.Content
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "flex flex-col-reverse gap-2 bg-none py-4 -mx-6 px-6 sm:flex-row sm:justify-end",
        className
      )}
      {...props}
    >
