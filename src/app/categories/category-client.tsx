...
interface AddProps { action: "add"; variant?: "default" | "icon"; onSave?: () => void }
interface EditProps { action: "edit"; category: Category; rules: CategoryRule[]; banks: Bank[]; onSave?: () => void; open?: boolean; onOpenChange?: (v: boolean) => void }
...
export function CategoryClient(props: Props) {
  ...
  const isDefault = isEdit ? props.category.isDefault : false;
  ...
  // Default categories (seeded from src/config/categories.ts) can never be edited, no matter how the dialog is triggered.
  const open = isDefault ? false : controlledOpen !== undefined ? controlledOpen : internalOpen;
  ...
  const [form, setForm] = useState({
    name:       cat?.name       ?? "",
    budgetType: cat?.budgetType ?? "willen",
  ...
  if (open !== wasOpen) {
    setWasOpen(open);
    if (open) setRulesOpen(false);
  }

  // New rule form state — matchType: "contains" | "word" | "exact"
  const [newRule, setNewRule] = useState({ namePattern: "", matchType: "contains" as "contains" | "word" | "exact", amount: "", direction: "", bankId: "" });
  const [ruleLoading, setRuleLoading] = useState(false);
  const [addRuleOpen, setAddRuleOpen] = useState(false);

  const [editingRule, setEditingRule] = useState<number | null>(null);
  const [editForm, setEditForm] = useState({ namePattern: "", matchType: "contains" as "contains" | "word" | "exact", amount: "", direction: "", bankId: "" });
  ...
  async function remove() {
    if (!confirm(`Verwijder categorie "${cat?.name}"? Alle gekoppelde transacties, terugkerende items, budgetdoelen en regels worden ook verwijderd.`)) return;
    setLoading(true);
    const res = await fetch("/api/categories", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
      setLoading(false);
      alert("Het verwijderen van de categorie is mislukt.");
      return;
    }
    setLoading(false);
    setOpen(false);
  ...
  async function addRule() {
    if (!newRule.namePattern && !newRule.amount && !newRule.bankId) return;
    const fields = {
    ...
    if (!isEdit) {
      ...
      setNewRule({ namePattern: "", matchType: "contains", amount: "", direction: "", bankId: "" });
      return;
    }
  ...
  return (
    <>
      {isEdit && isDefault ? null : isEdit && controlledOpen === undefined ? (
        <button
          onClick={openDialog}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90dvh] overflow-y-auto [scrollbar-gutter:stable]">
          <DialogHeader>
            <DialogTitle>{isEdit ? "Bewerken" : "Nieuwe categorie"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-6">
              <div className="flex-1">
                <PickerField label="Icoon">
                  <IconPicker value={form.icon} onChange={(v) => setForm((f) => ({ ...f, icon: v }))} previewColor={form.color} hideBrandTab />
                </PickerField>
              </div>
              {!(form.icon && isBrandIcon(form.icon)) && (
                <div className="flex-1">
                  <PickerField label="Kleur">
            ...
            <div className="rounded-md border border-foreground/15">
              ...
              >
                <span className="text-md text-foreground font-medium">
                  Automatische regels{rules.length > 0 ? ` (${rules.length})` : ""}
                </span>
                        ...
                        <div key={rule.id} className="py-2">
                          <div className="bg-foreground/3 rounded-lg h-12 p-3 flex items-center justify-between gap-2">
                            <div className="text-sm text-foreground/60 min-w-0">
                              {rule.namePattern && (
                                <span>
                                  Naam {rule.nameWholeWord ? "heel woord" : rule.nameWildcard ? "bevat" : "exact"}{" "}
                                  <span className="font-mono text-foreground">&ldquo;{rule.namePattern}&rdquo;</span>
                                </span>
                              )}
                              {rule.namePattern && rule.amount !== null && <span className="mx-1">+</span>}
                              {rule.amount !== null && rule.amount !== undefined && (
                                <span>
                                  bedrag <span className="font-mono text-foreground">{formatEur(rule.amount)}</span>
                                </span>
                              )}
                              {rule.direction && (
                                <span className="ml-1 text-xs">· {rule.direction === "income" ? "inkomst" : "uitgave"}</span>
                              )}
                              {rule.bankId && (() => {
            ...
            {addRuleOpen && (
              <div className="rounded-lg border border-foreground/15 p-3 space-y-2 bg-foreground/[0.02]">
                <p className="text-sm font-medium">Nieuwe regel</p>
                <Field label="Naam">
                  <Input
                ...
                <div className="flex gap-2 pt-1">
                  <Button size="lg" variant="default" onClick={submitAddRule}
                    disabled={ruleLoading || (!newRule.namePattern && !newRule.amount && !newRule.bankId)}
                    className="flex-1">
                    <Check className="size-4 mr-1" />Toevoegen
                  </Button>
                  <Button size="lg" variant="ghost" onClick={() => setAddRuleOpen(false)} className="flex-1">Annuleren</Button>
                </div>
              </div>
            )}
              ...
              <div className="rounded-lg border border-foreground/15 p-3 space-y-2 bg-foreground/[0.02]">
                ...
                <div className="flex gap-2 pt-1">
                  <Button size="lg" variant="default" onClick={() => editingRule !== null && saveEdit(editingRule)} className="flex-1">
                    <Check className="size-4 mr-1" />Opslaan
                  </Button>
                  <Button size="lg" variant="ghost" onClick={() => setEditingRule(null)} className="flex-1">Annuleren</Button>
                </div>
              </div>
            )}
...
function MatchTypePicker({
  ...
  const options: { value: "contains" | "word" | "exact"; label: string; title: string }[] = [
    { value: "contains", label: "Bevat", title: "Naam bevat de tekst (ook als deel van een woord)" },
    { value: "word", label: "Heel woord", title: "Naam bevat het als een heel woord (MCD matcht niet McDonalds)" },
    { value: "exact", label: "Exact", title: "Naam is precies gelijk aan de tekst" },
  ];
  return (
